...
func (bot *Bot) RemoveAllFilters(update tgbotapi.Update) {
	bot_in_db := bot.DB.GetBotByToken(bot.Bot.Token)
	if bot_in_db.BotToken == "" {
		// fmt.Println("here!")
		return
	...
	for filter := range chat.Filters {
		// text += "`" + filter + "`" + "\n"
		delete(chat.Filters, filter)
	}
	chats[update.Message.Chat.ID] = chat
	bot_in_db.Chats = chats
	bot.DB.UpdateBot(bot_in_db)
	msg := tgbotapi.NewMessage(update.Message.Chat.ID, "All Filters removed!")
	msg.ParseMode = "Markdown"
	bot.Bot.Send(msg)
}

// Work on thisssss line 373,375,384,391
func (bot *Bot) AddBlackList(update tgbotapi.Update) {
	// slice if " in message
	slices := strings.Split(update.Message.Text, "\"")
	// fmt.Println(slices, len(slices))
	if len(slices) > 2 {
		filter_text := slices[1]
		filter_response := slices[2]
		slices101 := strings.Split(slices[2], " ")
		if len(slices101) > 1 {
			filter_response = slices101[0]
			filter_time := slices101[1]
			_, _ = filter_response, filter_time
		}
		_ = filter_text

		// fmt.Println(filter_text, filter_response)
	} else {
		slices = strings.Split(update.Message.Text, " ")
		if len(slices) < 4 {
			filter_text := slices[1]
			filter_response := slices[2]
			filter_time := "0m"
			_, _, _ = filter_response, filter_time, filter_text

			// fmt.Println(filter_text, filter_response)
		} else if len(slices) > 3 {
			filter_text := slices[1]
			filter_response := slices[2]
			filter_time := slices[3]
			_, _, _ = filter_response, filter_time, filter_text
			// fmt.Println(filter_text, filter_response, filter_time)
		} else {
			bot.SendText(update.Message.Chat.ID, "Invalid command!")
			return
		}
	...
	if chat.ChatID == 0 {
		...
		bot_in_db.Chats = chats
		bot.DB.UpdateBot(bot_in_db)
		// bot.SendText(update.Message.Chat.ID, "No BlackList found in this chat!")
		// return
	}
	if len(slices) > 2 {
		filter_word := strings.ToLower(slices[1])
